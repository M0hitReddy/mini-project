...
function NewChatDialog() {
    ...
    const handleStartChat = () => {
        async function createConversation() {
            try {
                setNewChatLoading(true);
                const res = await axios.post('http://localhost:5000/chats/conversation', { members: selectedPeople, creator: user.id });
                console.log(res.data);
                setTimeout(() => {
                    setNewChatLoading(false);
                }, 1000)
            ...
            <Tooltip>
                <TooltipTrigger asChild>
                    <DialogTrigger asChild>
                        <Button variant="outline"><MessageCirclePlus /></Button>
                    </DialogTrigger>
                </TooltipTrigger>
                    ...
                    <TabsContent value="newgroup">
                        ...
                        <DialogHeader className={'flex flex-col gap-2 px-4 pt-2'}>
                            ...
                            <div className="relative">
                                ...
                                <Input placeholder="Search person.." disabled={newChatLoading ? true : false} className="pl-8" onChange={(e) => handleSearchChange(e)} />
                            </div>
                        </DialogHeader>
                        <Separator className='mt-4 mb-2' />

                        {(search === '') ?
                            (<DialogDescription className="px-4 py-2 h-40 min-h-52">Search for people to start a chat with</DialogDescription>) :
                            (fetching) ? (<DialogDescription className="px-4 py-2 h-40 flex justify-center items-center"><Loader className="mr-2 h-4 w-4 animate-spin" /></DialogDescription>) :
                                            ...
                                            {searchResults.map((result) => (result.id !== user.id) ? (
                                                <div key={result.id} className="space-x-2 border p-2 rounded-lg">
                                                    ...
                                                    >
                                                        ...
                                                        <Checkbox id={'newchat' + result.id} disabled={newChatLoading ? true : false} checked={result?.checked} onCheckedChange={() => handleCheckBoxChange(result)} />
                                                    </label>
                                                </div>
                                            ) : null)
                                            }
                        ...
                        <DialogFooter className={'px-4 pb-4'}>
                            <Button type="submit" disabled={newChatLoading ? true : false} variant="" className="border-primary w-full" onClick={() => handleStartChat()}>{newChatLoading ? <Loader className="mr-2 h-4 w-4 animate-spin" /> : 'Start Chat'}</Button>
                        </DialogFooter>
                    </TabsContent>
